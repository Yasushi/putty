#include <winsock2.h>
#include <windows.h>
#include <string.h>
#include "putty.h"
#include "iso2022.h"

int iso2022_win95flag;

static int
buflen (struct iso2022struct *p)
{
  return p->buflen ? p->buflen - p->bufoff : 0;
}

int
iso2022_buflen (struct iso2022_data *this)
{
  return buflen (&this->rcv);
}

int
iso2022_tbuflen (struct iso2022_data *this)
{
  return buflen (&this->trns);
}

void
iso2022_tbufclear (struct iso2022_data *this)
{
  this->trns.buflen = this->trns.bufoff = 0;
}

void
iso2022_settranschar (struct iso2022_data *this, int value)
{
  this->trns.transchar = value;
}

static unsigned char
getbuf (struct iso2022struct *p)
{
  return p->buf[p->bufoff++];
}

unsigned char
iso2022_getbuf (struct iso2022_data *this)
{
  return getbuf (&this->rcv);
}

unsigned char
iso2022_tgetbuf (struct iso2022_data *this)
{
  return getbuf (&this->trns);
}

int
iso2022_width (struct iso2022_data *this, wchar_t c)
{
  if (this->rcv.gr->type == UTF8) return mk_wcwidth_cjk (c);
  else return this->rcv.width;
}

static int
utf8len (unsigned char firstbyte)
{
  int j = 0;
  unsigned char c = firstbyte;

  if ((c & 0x80) == 0)
    j = 1;
  else if ((c & 0xe0) == 0xc0)
    j = 2;
  else if ((c & 0xf0) == 0xe0)
    j = 3;
  else if ((c & 0xf8) == 0xf0)
    j = 4;
  else if ((c & 0xfc) == 0xf8)
    j = 5;
  else if ((c & 0xfe) == 0xfc)
    j = 6;
  return j;
}

static void
setg_94_n (struct iso2022struct *q, struct g *p)
{
  p->len = 2;
  p->type = UNKNOWN;
  if (q->bufoff <= 3)
    {
      switch (q->buf[q->bufoff])
	{
	case '@': p->type = JISC6226_1978; break;
	case 'B':
	  if (q->jisx02081990flag)
	    q->jisx02081990flag = 0, p->type = JISX0208_1990;
	  else
	    p->type = JISX0208_1983;
	  break;
	case 'D': p->type = JISX0212_1990; break;
	case 'A': p->type = GB2312_80; break;
	case 'G': p->type = CSIC_SET1; break;
	case 'H': p->type = CSIC_SET2; break;
	case 'I': p->type = CSIC_SET3; break;
	case 'J': p->type = CSIC_SET4; break;
	case 'K': p->type = CSIC_SET5; break;
	case 'L': p->type = CSIC_SET6; break;
	case 'M': p->type = CSIC_SET7; break;
	case 'C': p->type = KSC5601_1987; break;
	case 'O': p->type = JISX0213_1; break;
	case 'P': p->type = JISX0213_2; break;
	case 'E':
	  p->type = UNSUPPORTED;
	  break;
	}
    }
}

static void
setg_96_n (struct iso2022struct *q, struct g *p)
{
  p->type = UNKNOWN;
  p->len = 1;
}

static void
setg_96_1 (struct iso2022struct *q, struct g *p)
{
  p->len = 1;
  p->type = UNKNOWN;
  if (q->bufoff == 2)
    {
      switch (q->buf[q->bufoff])
	{
	case 'A': p->type = ISO8859_1; break;
	case 'B': p->type = ISO8859_2; break;
	case 'C': p->type = ISO8859_3; break;
	case 'D': p->type = ISO8859_4; break;
	case 'L': p->type = ISO8859_5; break;
	case 'G': p->type = ISO8859_6; break;
	case 'F': p->type = ISO8859_7; break;
	case 'H': p->type = ISO8859_8; break;
	case 'M': p->type = ISO8859_9; break;
	case 'V': p->type = ISO8859_10; break;
	case 'Z':
	  p->type = UNSUPPORTED;
	  break;
	}
    }
}

static void
setg_94_1 (struct iso2022struct *q, struct g *p)
{
  p->len = 1;
  p->type = UNKNOWN;
  if (q->bufoff == 2)
    {
      switch (q->buf[q->bufoff])
	{
	case 'I': p->type = JISX0201_KATAKANA; break;
	case 'J': p->type = JISX0201_ROMAN; break;
	case 'B': p->type = US_ASCII; break;
	case '0': p->type = VT100GRAPHICS; break;
	case '@': p->type = ISO646_1973IRV; break;
	case 'A': p->type = BS4730; break;
	case 'C': p->type = NATS_PRIMARY_FINLAND_SWEDEN; break;
	case 'D': p->type = NATS_SECONDARY_FINLAND_SWEDEN; break;
	case 'E': p->type = NATS_PRIMARY_DENMARK_NORWAY; break;
	case 'F': p->type = NATS_SECONDARY_DENMARK_NORWAY; break;
	case 'G': p->type = SEN850200ANNB; break;
	case 'H': p->type = SEN850200ANNC; break;
	case 'K': p->type = DIN66003; break;
	case 'L': p->type = ISO646_PORTUGUESE; break;
	case 'N': p->type = ISO5427; break;
	case 'R': p->type = NFZ62010_1973; break;
	case 'U': p->type = LATIN_GREEK_HONEYWELL_BULL; break;
	case 'X': p->type = ISO5428; break;
	case 'Y': p->type = ISO646_ITALIAN; break;
	case 'Z': p->type = ISO646_SPANISH; break;
	case '[': p->type = ECMA_GREEK; break;
	case '\\': p->type = LATIN_GREEK; break;
	case 'M':
	case 'O':
	  p->type = UNSUPPORTED;
	  break;
	}
    }
}

static void
translate (struct iso2022struct *q, struct g *p)
{
  WCHAR buf2[100];
  int i;

  q->buflen = 0;
  q->width = p->len;
  switch (p->type)
    {
    case US_ASCII:
      buf2[0] = q->buf[0] & 0x7f;
      buf2[1] = 0;
      q->buflen = WideCharToMultiByte
	(CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0);
      if (q->buflen)
	q->buflen--;
      break;
    case JISX0201_ROMAN:
      buf2[0] = q->buf[0] & 0x7f;
      buf2[1] = 0;
      if (buf2[0] == 0x5c) buf2[0] = 0xa5;
      if (buf2[0] == 0x7e) buf2[0] = 0x203e;
      q->buflen =
	WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0);
      if (q->buflen)
	q->buflen--;
      break;
    case JISX0201_KATAKANA:
      buf2[0] = q->buf[0] & 0x7f;
      buf2[0] += 0xff40;
      buf2[1] = 0;
      q->buflen =
	WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0);
      if (q->buflen)
	q->buflen--;
      break;
#define A(a) q->buf[0] &= 0x7f; buf2[0] = q->buf[0] >= 0x20 ? a[q->buf[0] - 0x20] : 0; buf2[1] = 0; \
q->buflen = WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0); \
if (q->buflen) q->buflen--;
    case ISO8859_1: A(iso_8859_1); break;
    case ISO8859_2: A(iso_8859_2); break;
    case ISO8859_3: A(iso_8859_3); break;
    case ISO8859_4: A(iso_8859_4); break;
    case ISO8859_5: A(iso_8859_5); break;
    case ISO8859_6: A(iso_8859_6); break;
    case ISO8859_7: A(iso_8859_7); break;
    case ISO8859_8: A(iso_8859_8); break;
    case ISO8859_9: A(iso_8859_9); break;
    case ISO8859_10: A(iso_8859_10); break;
    case ISO646_1973IRV: A(iso646_1973irv); break;
    case BS4730: A(bs4730); break;
    case NATS_PRIMARY_FINLAND_SWEDEN: A(nats_primary_finland_sweden); break;
    case NATS_SECONDARY_FINLAND_SWEDEN:A(nats_secondary_finland_sweden); break;
    case NATS_PRIMARY_DENMARK_NORWAY: A(nats_primary_denmark_norway); break;
    case NATS_SECONDARY_DENMARK_NORWAY:A(nats_secondary_denmark_norway); break;
    case SEN850200ANNB: A(sen850200annb); break;
    case SEN850200ANNC: A(sen850200annc); break;
    case DIN66003: A(din66003); break;
    case ISO646_PORTUGUESE: A(iso646_portuguese); break;
    case ISO5427: A(iso5427); break;
    case NFZ62010_1973: A(nfz62010_1973); break;
    case LATIN_GREEK_HONEYWELL_BULL: A(latin_greek_honeywell_bull); break;
    case ISO5428: A(iso5428); break;
    case ISO646_ITALIAN: A(iso646_italian); break;
    case ISO646_SPANISH: A(iso646_spanish); break;
    case ECMA_GREEK: A(ecma_greek); break;
    case LATIN_GREEK: A(latin_greek); break;
    case VT100GRAPHICS:
      buf2[0] = q->buf[0] & 0x7f;
      buf2[0] |= 0xd900;        /* == ATTR_LINEDRW (defined in putty.h) */
      buf2[1] = 0;
      q->buf[3] = 0;
      q->buf[2] = 0x80 | (buf2[0] & 0x3f), buf2[0] >>= 6;
      q->buf[1] = 0x80 | (buf2[0] & 0x3f), buf2[0] >>= 6;
      q->buf[0] = 0xe0 | (buf2[0] & 0xf);
      q->buflen = 3;
      break;
      q->buf[0] &= 0x7f; buf2[0] = q->buf[0] >= 0x20 ? q->buf[0] : 0;
      if (q->buf[0] >= 0x60 && q->buf[0] <= 0x7f)
        buf2[0] = unitab_xterm_std[q->buf[0] - 0x60];
      q->buflen = WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0);
      if (q->buflen) q->buflen--;
      break;
#undef A
#define A(a) q->buf[0] &= 0x7f;q->buf[1] &= 0x7f;q->buflen = 0; \
if (q->buf[0] >= 0x21 && q->buf[0] <= 0x7e && q->buf[1] >= 0x21 && q->buf[1] <= 0x7e) \
{buf2[0] = a[q->buf[0] - 0x21][q->buf[1] - 0x21];buf2[1] = 0; \
q->buflen = WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0); \
if (q->buflen)q->buflen--;}
#define B(a) if((q->buf[0]&0x7f)>=0x30){A(a);}else{ \
q->buf[0]&=0x7f;q->buf[1]&=0x7f;q->buf[2]=q->buf[0]-0x21; \
q->buf[3]=q->buf[1]-0x21+((q->buf[2]%2)?94:0)+64; \
q->buf[0]=q->buf[2]/2+129;q->buf[1]=q->buf[3]+((q->buf[3]>=0x7f)?1:0); \
q->buf[2]=0;MultiByteToWideChar(932,0,q->buf,-1,buf2,100); \
q->buflen = WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0); \
if (q->buflen)q->buflen--;}
    case JISC6226_1978: B(jisc6226_1978); break;
    case JISX0208_1983: B(jisx0208_1983); break;
    case JISX0208_1990: B(jisx0208_1990); break;
    case JISX0212_1990: A(jisx0212_1990); break;
    case JISX0213_1: A(jisx0213_1); break;
    case JISX0213_2: A(jisx0213_2); break;
    case GB2312_80: A(gb2312_80); break;
    case CSIC_SET1: A(csic_set1); break;
    case CSIC_SET2: A(csic_set2); break;
    case CSIC_SET3: A(csic_set3); break;
    case CSIC_SET4: A(csic_set4); break;
    case CSIC_SET5: A(csic_set5); break;
    case CSIC_SET6: A(csic_set6); break;
    case CSIC_SET7: A(csic_set7); break;
    case KSC5601_1987: A(ksc5601_1987); break;
#undef B
#undef A
    case MS_KANJI:
      q->width = (q->buf[0] >= 160 && q->buf[0] < 224) ? 1 : 2;
      q->buf[q->width] = 0;
      MultiByteToWideChar (932, 0, q->buf, -1, buf2, 100);
      q->buflen =
	WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0);
      break;
    case BIG5:
      q->width = 2;
      q->buf[q->width] = 0;
      MultiByteToWideChar (950, 0, q->buf, -1, buf2, 100);
      q->buflen =
	WideCharToMultiByte (CP_UTF8, 0, buf2, -1, q->buf, 100, 0, 0);
      break;
    }
  if (!q->buflen)
    {
      for (i = 0 ; i < p->len ; i++)
	q->buf[i] = '?';
      q->buf[i] = 0;
      q->buflen = p->len;
      q->width = 1;
    }
  q->bufoff = 0;
  if (q->ssl)
    q->gl = q->ssl, q->gr = q->ssr, q->ssl = 0, q->ssr = 0;
}

static int
try0 (struct g *p, WCHAR buf2, unsigned char *buf)
{
  int i, j;

  switch (p->type)
    {
    case US_ASCII:
      if (buf2 >= 0x80)
	return 0;
      buf[0] = buf2 & 0x7f;
      buf[1] = 0;
      return 1;
    case JISX0201_ROMAN:
      if (buf2 == 0xa5)
	buf2 = 0x5c;
      if (buf2 == 0x203e)
	buf2 = 0x7e;
      if (buf2 >= 0x80)
	return 0;
      buf[0] = buf2 & 0x7f;
      buf[1] = 0;
      return 1;
    case JISX0201_KATAKANA:
      if (buf2 < 0xff61)
	return 0;
      if (buf2 > 0xff9f)
	return 0;
      buf[0] = buf2 - 0xff40;
      buf[1] = 0;
      return 1;
#define A(a) for(i=0;i<96;i++)if(a[i]==buf2)break;if(i==96)return 0; \
buf[0]=i+32;buf[1]=0;return 1;
    case ISO8859_1: A(iso_8859_1); break;
    case ISO8859_2: A(iso_8859_2); break;
    case ISO8859_3: A(iso_8859_3); break;
    case ISO8859_4: A(iso_8859_4); break;
    case ISO8859_5: A(iso_8859_5); break;
    case ISO8859_6: A(iso_8859_6); break;
    case ISO8859_7: A(iso_8859_7); break;
    case ISO8859_8: A(iso_8859_8); break;
    case ISO8859_9: A(iso_8859_9); break;
    case ISO8859_10: A(iso_8859_10); break;
    case ISO646_1973IRV: A(iso646_1973irv); break;
    case BS4730: A(bs4730); break;
    case NATS_PRIMARY_FINLAND_SWEDEN: A(nats_primary_finland_sweden); break;
    case NATS_SECONDARY_FINLAND_SWEDEN:A(nats_secondary_finland_sweden); break;
    case NATS_PRIMARY_DENMARK_NORWAY: A(nats_primary_denmark_norway); break;
    case NATS_SECONDARY_DENMARK_NORWAY:A(nats_secondary_denmark_norway); break;
    case SEN850200ANNB: A(sen850200annb); break;
    case SEN850200ANNC: A(sen850200annc); break;
    case DIN66003: A(din66003); break;
    case ISO646_PORTUGUESE: A(iso646_portuguese); break;
    case ISO5427: A(iso5427); break;
    case NFZ62010_1973: A(nfz62010_1973); break;
    case LATIN_GREEK_HONEYWELL_BULL: A(latin_greek_honeywell_bull); break;
    case ISO5428: A(iso5428); break;
    case ISO646_ITALIAN: A(iso646_italian); break;
    case ISO646_SPANISH: A(iso646_spanish); break;
    case ECMA_GREEK: A(ecma_greek); break;
    case LATIN_GREEK: A(latin_greek); break;
#undef A
#define A(a) for(j=94,i=0;i<94&&j==94;i++)for(j=0;j<94;j++)if(a[i][j]==buf2)break; \
if(j!=94){buf[0]=i+0x20;buf[1]=j+0x21;buf[2]=0;return 1;}
#define B {WCHAR a[]={buf2,0};BOOL b; \
 WideCharToMultiByte(932,0,a,-1,buf,5,0,&b); \
 if (buf[0]<129||buf[0]>=0xf0||(buf[0]>=160&&buf[0]<224))return 0; \
 if(!b){if(buf[0]>=224)buf[0]-=224-160;buf[0]-=129;if(buf[1]>=0x7f)buf[1]--; \
 buf[1]-=64;buf[0]=buf[0]*2+(buf[1]>=94);if(buf[1]>=94)buf[1]-=94;buf[2]=0; \
 buf[0]+=0x21;buf[1]+=0x21;return 1;}}

    case JISC6226_1978: A(jisc6226_1978); B; break;
    case JISX0208_1983: A(jisx0208_1983); B; break;
    case JISX0208_1990: A(jisx0208_1990); B; break;
    case JISX0212_1990: A(jisx0212_1990); break;
    case JISX0213_1: A(jisx0213_1); B; break;
    case JISX0213_2: A(jisx0213_2); break;
    case GB2312_80: A(gb2312_80); break;
    case CSIC_SET1: A(csic_set1); break;
    case CSIC_SET2: A(csic_set2); break;
    case CSIC_SET3: A(csic_set3); break;
    case CSIC_SET4: A(csic_set4); break;
    case CSIC_SET5: A(csic_set5); break;
    case CSIC_SET6: A(csic_set6); break;
    case CSIC_SET7: A(csic_set7); break;
    case KSC5601_1987: A(ksc5601_1987); break;
    case MS_KANJI:
      {
	WCHAR a[] = { buf2, 0 };
	BOOL b;

	WideCharToMultiByte (932, 0, a, -1, buf, 5, 0, &b);
	if (!b)
	  return 1;
      }
      break;
    case BIG5:
      {
	WCHAR a[] = { buf2, 0 };
	BOOL b;

	WideCharToMultiByte (950, 0, a, -1, buf, 5, 0, &b);
	if (!b)
	  return 1;
      }
      break;
#undef B
#undef A
    case VT100GRAPHICS:         /* ? */
      for(i=0;i<32;i++)
        if(unitab_xterm_std[i]==buf2)
          break;
      if(i==32)
        return 0;
      buf[0]=i + 0x60;
      buf[1]=0;
      return 1;
    }
  return 0;
}

static void
clearesc (struct iso2022struct *q)
{
  q->esc = 0;
}

void
iso2022_clearesc (struct iso2022_data *this)
{
  clearesc (&this->rcv);
}

static void
put (struct iso2022struct *q, unsigned char c)
{
  int f = 0;

  if (q->buflen)
    q->buflen = q->bufoff = 0;
  q->buf[q->bufoff] = c;
  if (q->esc || q->gr->type == UTF8)
    f = 2;
  else
    {
      switch (q->bufoff)
	{
	case 0:
	  if (q->gr->type == MS_KANJI && c >= 129 && c <= 252)
	    {
	      f = 1;
	      break;
	    }
	  switch (c)
	    {
	    case 0xf:
	      q->gl = &q->g0;
	      break;
	    case 0xe:
	      q->gl = &q->g1;
	      break;
	    case 0x8e:
	      q->ssl = q->gl;
	      q->ssr = q->gr;
	      q->gl = &q->g2;
	      q->gr = &q->g2;
	      break;
	    case 0x8f:
	      q->ssl = q->gl;
	      q->ssr = q->gr;
	      q->gl = &q->g3;
	      q->gr = &q->g3;
	      break;
	    default:
	      if ((c >= 0x20 && c <= 0x7f) ||
		  (c >= 0xa0 && c <= 0xff) ||
		  c == 0x1b)
		f = 1;
	      else
		{
		  if (c >= 0x80 && c <= 0x9f)
		    {
		      q->buf[0] = 0x1b;
		      q->buf[1] = c - 0x80 + 0x40;
		      q->bufoff++;
		    }
		  f = 2;
		}
	    }
	  break;
	case 1:
	  switch (q->buf[0])
	    {
	    case 0x1b:
	      switch (c)
		{
		case '(':
		case ')':
		case '*':
		case '+':
		case '-':
		case '.':
		case '/':
		case '$':
		case '&':
		  f = 1;
		  break;
		case 0x6e:
		  q->gl = &q->g2;
		  f = 3;
		  break;
		case 0x6f:
		  q->gl = &q->g3;
		  f = 3;
		  break;
		case 0x7e:
		  q->gr = &q->g1;
		  f = 3;
		  break;
		case 0x7d:
		  q->gr = &q->g2;
		  f = 3;
		  break;
		case 0x7c:
		  q->gr = &q->g3;
		  f = 3;
		  break;
		case 0x4e:
		  q->ssl = q->gl;
		  q->ssr = q->gr;
		  q->gl = &q->g2;
		  q->gr = &q->g2;
		  f = 3;
		  break;
		case 0x4f:
		  q->ssl = q->gl;
		  q->ssr = q->gr;
		  q->gl = &q->g3;
		  q->gr = &q->g3;
		  f = 3;
		  break;
		default:
		  f = 2;
		  q->esc = 1;
		}
	      break;
	    default:
	      f = 1;
	    }
	  break;
	default:
	  if (q->buf[0] == 0x1b)
	    {
	      if (c >= 0x30 && c <= 0x7e)
		{
		  f = 3;
		  switch (q->buf[1])
		    {
		    case '(': setg_94_1 (q, &q->g0); break;
		    case ')': setg_94_1 (q, &q->g1); break;
		    case '*': setg_94_1 (q, &q->g2); break;
		    case '+': setg_94_1 (q, &q->g3); break;
		    case '-': setg_96_1 (q, &q->g1); break;
		    case '.': setg_96_1 (q, &q->g2); break;
		    case '/': setg_96_1 (q, &q->g3); break;
		    case '$':
		      switch (q->buf[2])
			{
			case '(': setg_94_n (q, &q->g0); break;
			case ')': setg_94_n (q, &q->g1); break;
			case '*': setg_94_n (q, &q->g2); break;
			case '+': setg_94_n (q, &q->g3); break;
			case '-': setg_96_n (q, &q->g1); break;
			case '.': setg_96_n (q, &q->g2); break;
			case '/': setg_96_n (q, &q->g3); break;
			default:
			  if (c == '@' || c == 'A' || c == 'B')
			    setg_94_n (q, &q->g0);
			  else
			    f = 2;
			}
		      break;
		    case '&':
		      if (c == '@')
			q->jisx02081990flag = 1;
		      break;
		    default:
		      f = 2;
		    }
		}
	      else if (c >= 0x20 && c <= 0x2f)
		f = 1;
	      else
		f = 2;
	    }
	  else
	    f = 1;
	  break;
	}
    }
  if (f)
    {
      q->bufoff++;
      if (q->bufoff >= 8)
	f = 2;
      switch (f)
	{
	case 1:
	  if (q->buf[0] == 0x1b)
	    break;
	  if (q->gr->type == MS_KANJI && q->buf[0] >= 129 && q->buf[0] <= 252)
	    goto mskanji;
	  if ((q->buf[q->bufoff - 1] & 0x7f) < 0x20)
	    {
	      q->buf[0] = q->buf[q->bufoff - 1];
	      q->bufoff = 1;
	    }
	  else
	    {
	      if ((q->buf[0] & 0x7f) >= 0x20 && (q->buf[0] & 0x7f) <= 0x7f)
		{
		  if (q->buf[0] & 0x80)
		    {
		    mskanji:
		      if (q->gr->len == q->bufoff ||
			  (q->gr->type == MS_KANJI &&
			   q->buf[0] >= 160 && q->buf[0] < 224))
			translate (q, q->gr);
		    }
		  else
		    {
		      if (q->gl->len == q->bufoff)
			translate (q, q->gl);
		    }
		  break;
		}
	    }
	  /* fall through */
	case 2:
	  q->buf[q->buflen = q->bufoff] = 0;
	  q->bufoff = 0;
	  break;
	case 3:
	  q->bufoff = 0;
	  break;
	}
    }
  if (q->lockgr && !q->ssl)
    q->gr = &q->lgr;
}

static int
try1 (struct iso2022struct *q, WCHAR buf2, unsigned char *p,
      unsigned char *designate, unsigned char *invoke,
      unsigned char *beforec0, int bits)
{
  struct iso2022struct saveq;

  saveq = *q;
  if (try0 (q->gl, buf2, p))
    return 1;
  if (bits == 7)
    return 0;
  *q = saveq;
  if (!try0 (q->gr, buf2, p))
    return 0;
  if (q->gr->type != MS_KANJI && q->gr->type != BIG5)
    while (*p)
      *p++ |= 0x80;
  return 1;
}

static int
try2 (struct iso2022struct *q, WCHAR buf2, unsigned char *p,
      unsigned char *designate, unsigned char *invoke,
      unsigned char *beforec0, int bits)
{
  struct iso2022struct saveq;
  unsigned char *pp, *ppp;

  saveq = *q;
  ppp = invoke;
  while (*ppp)
    {
      pp = p;
      q->esc = 0;
      q->buflen = 1;
      while (*ppp)
	put (q, *pp++ = *ppp++);
      ppp++;
      if (try1 (q, buf2, pp, designate, invoke, beforec0, bits))
	{
	  if (q->ssl)
	    {
	      q->gl = q->ssl, q->gr = q->ssr, q->ssl = 0, q->ssr = 0;
	      if (q->ssgr)
		while (*pp)
		  *pp++ |= 0x80;
	    }
	  return 1;
	}
      *q = saveq;
    }
  return 0;
}

static int
try3 (struct iso2022struct *q, WCHAR buf2, unsigned char *p,
      unsigned char *designate, unsigned char *invoke,
      unsigned char *beforec0, int bits)
{
  struct iso2022struct saveq;
  unsigned char *pp, *ppp;

  saveq = *q;
  ppp = designate;
  while (*ppp)
    {
      pp = p;
      q->esc = 0;
      q->buflen = 1;
      while (*ppp)
	put (q, *pp++ = *ppp++);
      ppp++;
      if (try1 (q, buf2, pp, designate, invoke, beforec0, bits))
	{
	  if (q->ssl)
	    {
	      q->gl = q->ssl, q->gr = q->ssr, q->ssl = 0, q->ssr = 0;
	      if (q->ssgr)
		while (*pp)
		  *pp++ |= 0x80;
	    }
	  return 1;
	}
      *q = saveq;
    }
  return 0;
}

static int
try4 (struct iso2022struct *q, WCHAR buf2, unsigned char *p,
      unsigned char *designate, unsigned char *invoke,
      unsigned char *beforec0, int bits)
{
  struct iso2022struct saveq;
  unsigned char *pp, *ppp;

  saveq = *q;
  ppp = designate;
  while (*ppp)
    {
      pp = p;
      q->esc = 0;
      q->buflen = 1;
      while (*ppp)
	put (q, *pp++ = *ppp++);
      ppp++;
      if (try2 (q, buf2, pp, designate, invoke, beforec0, bits))
	return 1;
      *q = saveq;
    }
  return 0;
}

static void
transmit (struct iso2022_data *this, struct iso2022struct *q, unsigned char c)
{
  if (q->buflen)
    q->buflen = q->bufoff = 0;
  q->buf[q->bufoff++] = c;
  if (q->gr->type == UTF8)
    {
      q->buflen = q->bufoff;
      q->bufoff = 0;
      return;
    }
  if ((q->bufoff == 1 && (q->buf[0] & 0x80) == 0) ||
      (q->bufoff == 2 && (q->buf[0] & 0xe0) == 0xc0) ||
      (q->bufoff == 3 && (q->buf[0] & 0xf0) == 0xe0) ||
      (q->bufoff == 4 && (q->buf[0] & 0xf8) == 0xf0) ||
      (q->bufoff == 5 && (q->buf[0] & 0xfc) == 0xf8) ||
      (q->bufoff == 6 && (q->buf[0] & 0xfe) == 0xfc))
    {
      WCHAR buf2[100];
      unsigned char *p1, *p2, *p3, *p4, buf3[100];
      int bits;

      p1 = this->initstring;
      p1 += strlen (p1) + 1;
      p1 += strlen (p1) + 1;
      p2 = p1;
      while (strlen (p2))
	p2 += strlen (p2) + 1;
      p2++;
      p3 = p2;
      while (strlen (p3))
	p3 += strlen (p3) + 1;
      p3++;
      p4 = p3;
      while (strlen (p4))
	p4 += strlen (p4) + 1;
      p4++;
      bits = *p4;
      if (q->buf[0] >= 0x20 && q->buf[0] != 0x7f)
	{
	  q->buf[q->bufoff] = 0;
	  MultiByteToWideChar (CP_UTF8, 0, q->buf, -1, buf2, 100);
	  for (;;)
	    {
	      if (!try1 (q, buf2[0], buf3, p1, p2, p3, bits))
		if (!try2 (q, buf2[0], buf3, p1, p2, p3, bits))
		  if (!try3 (q, buf2[0], buf3, p1, p2, p3, bits))
		    if (!try4 (q, buf2[0], buf3, p1, p2, p3, bits))
		      {
			if (buf2[0] == L'?')
			  buf3[0] = '?', buf3[1] = 0;
			else
			  {
			    buf2[0] = L'?';
			    continue;
			  }
		      }
	      break;
	    }
	  strcpy (q->buf, buf3);
	  q->buflen = strlen (q->buf);
	  q->bufoff = 0;
	}
      else
	{
	  unsigned char c = q->buf[0];
	  struct iso2022struct saveq;
	  unsigned char *pp, *ppp, *p5;

	  ppp = p3;
	  pp = q->buf;
	  while (*ppp)
	    {
	      p5 = pp;
	      saveq = *q;
	      q->esc = 0;
	      q->buflen = 1;
	      while (*ppp)
		put (q, *pp++ = *ppp++);
	      ppp++;
	      if (!memcmp (&q->g0, &saveq.g0, sizeof (q->g0)) &&
		  !memcmp (&q->g1, &saveq.g1, sizeof (q->g1)) &&
		  !memcmp (&q->g2, &saveq.g2, sizeof (q->g2)) &&
		  !memcmp (&q->g3, &saveq.g3, sizeof (q->g3)) &&
		  q->gl == saveq.gl &&
		  q->gr == saveq.gr)
		*q = saveq, pp = p5;
	    }
	  *pp++ = c;
	  *pp = 0;
	  q->buflen = strlen (q->buf);
	  q->bufoff = 0;
	  if (!c)
	    q->buflen++;
	}
    }
}

static void
transmit2 (struct iso2022_data *this, struct iso2022struct *q, unsigned char c)
{
  if (q->transchar) {
	transmit (this, q, c);
	return;
  }
  if (q->buflen)
    q->buflen = q->bufoff = 0;
  if (c < 0x20 || c >= 0x7f)
    {
      unsigned char *p1, *p2, *p3;

      p1 = this->initstring;
      p1 += strlen (p1) + 1;
      p1 += strlen (p1) + 1;
      p2 = p1;
      while (strlen (p2))
	p2 += strlen (p2) + 1;
      p2++;
      p3 = p2;
      while (strlen (p3))
	p3 += strlen (p3) + 1;
      p3++;
        {
	  struct iso2022struct saveq;
	  unsigned char *pp, *ppp, *p5;

	  ppp = p3;
	  pp = q->buf;
	  while (*ppp)
	    {
	      p5 = pp;
	      saveq = *q;
	      q->esc = 0;
	      q->buflen = 1;
	      while (*ppp)
		put (q, *pp++ = *ppp++);
	      ppp++;
	      if (!memcmp (&q->g0, &saveq.g0, sizeof (q->g0)) &&
		  !memcmp (&q->g1, &saveq.g1, sizeof (q->g1)) &&
		  !memcmp (&q->g2, &saveq.g2, sizeof (q->g2)) &&
		  !memcmp (&q->g3, &saveq.g3, sizeof (q->g3)) &&
		  q->gl == saveq.gl &&
		  q->gr == saveq.gr)
		*q = saveq, pp = p5;
	    }
	  *pp++ = c;
	  *pp = 0;
	  q->buflen = strlen (q->buf);
	  q->bufoff = 0;
	  if (!c)
	    q->buflen++;
	}
    }
  else {
	q->buf[0] = c;
	q->buflen = 1;
  }
}

void
iso2022_put (struct iso2022_data *this, unsigned char c)
{
  put (&this->rcv, c);
}

void
iso2022_transmit (struct iso2022_data *this, unsigned char c)
{
  transmit2 (this, &this->trns, c);
}

static void
init (struct iso2022struct *q)
{
  q->g0.len = 1, q->g0.type = US_ASCII;
  q->g1.len = 1, q->g1.type = US_ASCII;
  q->g2.len = 1, q->g2.type = US_ASCII;
  q->g3.len = 1, q->g3.type = US_ASCII;
#if 0
  if (!strcmp (p, "EUC-JP"))
    {
      q->g1.len = 2, q->g1.type = JISX0208_1983;
      q->g2.len = 1, q->g2.type = JISX0201_KATAKANA;
      q->g3.len = 2, q->g3.type = JISX0212_1990;
    }
#endif
  q->gl = &q->g0;
  q->gr = &q->g1;
  q->ssl = 0;
  q->ssr = 0;
  q->jisx02081990flag = 0;
  q->buflen = q->bufoff = 0;
  q->esc = 0;
  q->lockgr = 0;
  q->ssgr = 0;
  q->transchar = 0;
}

/*
  examples
  euc-jp:
  > 1b2842 1b242942 1b2a49 1b242b44 0f 1b7e 00
  > 1b2842 1b242942 1b2a49 1b242b44 0f 1b7e 00
  > 00
  > 8e 00 8f 00 00
  > 00
  > 08
  iso-2022-jp:
  > 1b2842 1b2949 0f 1b7e 00
  > 1b2842 1b2949 0f 1b7e 00
  > 1b2842 00 1b242842 00 1b 24 28 44 00 00
  > 0f 00 0e 00 00
  > 1b2842 00 0f 00 00
  > 07
 */
/* mode
   0: always initialize
   1: initialize if initstring changed
   2: only check whether initstring is correct or not
   return value
   0: success
   -1: error
*/

int
iso2022_init (struct iso2022_data *this, char *p, int mode)
{
  int i, f, j, k;
  int tmp_lockgr, tmp_mskanji, tmp_big5, tmp_win95flag, tmp_ssgr, tmp_utf8;
  unsigned char *init_string = this->initstring;

  if (!lstrcmpi (p, "euc-jp"))
    p = "iso2022 lockgr euc-jp";
  else if (!lstrcmpi (p, "iso-2022-jp"))
    p = "iso2022 "
      "1b28420f00 1b28420f00 1b2842001b2442001b2428440000 00 1b28420000 07";
  else if (!lstrcmpi (p, "MS_Kanji") || !lstrcmpi (p, "Shift_JIS"))
    p = "iso2022 lockgr MS_Kanji";
  else if (!lstrcmpi (p, "big5"))
    p = "iso2022 big5";
  else if (!lstrcmpi (p, "euc-kr"))
    p = "iso2022 euc-kr";
  else if (!lstrcmpi (p, "euc-cn"))
    p = "iso2022 euc-cn";
  else if (!lstrcmpi (p, "euc-tw"))
    p = "iso2022 euc-tw";
  else if (!lstrcmpi (p, "utf-8 (cjk)"))
    p = "iso2022 utf-8";
  if (strnicmp (p, "iso2022 ", 8))
    return -1;
  p += 8;
  {
    unsigned char initstring[512];
    OSVERSIONINFO ovi;

    ovi.dwOSVersionInfoSize = sizeof (OSVERSIONINFO);
    GetVersionEx (&ovi);
    tmp_win95flag = (ovi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS);
    tmp_lockgr = tmp_mskanji = tmp_big5 = tmp_ssgr = tmp_utf8 = 0;
    for (;;)
      {
	if (*p == ' ')
	  {
	    p++;
	    continue;
	  }
	if (!strnicmp (p, "w95", 3))
	  {
	    tmp_win95flag = 1;
	    p += 3;
	    if (!strnicmp (p, "off", 3))
	      {
		tmp_win95flag = 0;
		p += 3;
	      }
	    continue;
	  }
	if (!strnicmp (p, "lockgr", 6))
	  {
	    tmp_lockgr = 1;
	    p += 6;
	    continue;
	  }
	if (!strnicmp (p, "ms_kanji", 8))
	  {
	    tmp_mskanji = 1;
	    p += 8;
	    continue;
	  }
	if (!strnicmp (p, "big5", 4))
	  {
	    tmp_big5 = 1;
	    p += 4;
	    continue;
	  }
	if (!strnicmp (p, "ssgr", 4))
	  {
	    tmp_ssgr = 1;
	    p += 4;
	    continue;
	  }
	if (!strnicmp (p, "utf-8", 5))
	  {
	    tmp_utf8 = 1;
	    p += 5;
	    continue;
	  }
	if (!lstrcmpi (p, "euc-jp"))
	  {
	    p =
	      /* receiving */
	      "1b2842"		/* G0: US ASCII */
	      /*"1b242942"*/	/* G1: JIS X0208-1983 */
	      "1b26401b242942"	/* G1: JIS X0208-1990 */
	      "1b2a49"		/* G2: JIS X0201 KATAKANA */
	      "1b242b44"	/* G3: JIS X0212-1990 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* transmitting */
	      "1b2842"		/* G0: US ASCII */
	      /*"1b242942"*/	/* G1: JIS X0208-1983 */
	      "1b26401b242942"	/* G1: JIS X0208-1990 */
	      "1b2a49"		/* G2: JIS X0201 KATAKANA */
	      "1b242b44"	/* G3: JIS X0212-1990 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* designating */
	      "00"
	      /* invoking */
	      "8e00"		/* SS2 */
	      "8f00"		/* SS3 */
	      "00"
	      /* before C0 control characters */
	      "00"
	      /* 7bit/8bit */
	      "08";
	    tmp_ssgr = 1;
	    break;
	  }
	if (!lstrcmpi (p, "euc-kr"))
	  {
	    p =
	      /* receiving */
	      "1b2842"		/* G0: US ASCII */
	      "1b242943"		/* G1: KS C 5601-1987 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* transmitting */
	      "1b2842"		/* G0: US ASCII */
	      "1b242943"		/* G1: KS C 5601-1987 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* designating */
	      "00"
	      /* invoking */
	      "8e00"		/* SS2 */
	      "8f00"		/* SS3 */
	      "00"
	      /* before C0 control characters */
	      "00"
	      /* 7bit/8bit */
	      "08";
	    break;
	  }
	if (!lstrcmpi (p, "euc-cn"))
	  {
	    p =
	      /* receiving */
	      "1b2842"		/* G0: US ASCII */
	      "1b242941"		/* G1: GB 2312-80 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* transmitting */
	      "1b2842"		/* G0: US ASCII */
	      "1b242941"		/* G1: GB 2312-80 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* designating */
	      "00"
	      /* invoking */
	      "8e00"		/* SS2 */
	      "8f00"		/* SS3 */
	      "00"
	      /* before C0 control characters */
	      "00"
	      /* 7bit/8bit */
	      "08";
	    break;
	  }
	if (!lstrcmpi (p, "euc-tw"))
	  {
	    p =
	      /* receiving */
	      "1b2842"		/* G0: US ASCII */
	      "1b242947"		/* G1: CSIC SET 1 */
	      "1b242b48"		/* G3: CSIC SET 2 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* transmitting */
	      "1b2842"		/* G0: US ASCII */
	      "1b242947"		/* G1: CSIC SET 1 */
	      "1b242b48"		/* G3: CSIC SET 2 */
	      "0f"		/* GL: G0 */
	      "1b7e"		/* GR: G1 */
	      "00"
	      /* designating */
	      "00"
	      /* invoking */
	      "8e00"		/* SS2 */
	      "8f00"		/* SS3 */
	      "00"
	      /* before C0 control characters */
	      "00"
	      /* 7bit/8bit */
	      "08";
	    break;
	  }
	break;
      }
    for (f = 0, i = 0 ; *p ; p++)
      {
	j = -1;
	if ('0' <= *p && *p <= '9')
	  j = *p - '0';
	if ('A' <= *p && *p <= 'F')
	  j = *p - 'A' + 10;
	if ('a' <= *p && *p <= 'f')
	  j = *p - 'a' + 10;
	if (j >= 0)
	  {
	    if (f)
	      {
		if (i >= 500)
		  return -1;
		k += j;
		initstring[i++] = k;
		f = 0;
	      }
	    else
	      {
		k = j * 16;
		f = 1;
	      }
	  }
      }
    if (tmp_mskanji && tmp_big5)
      return -1;
    if (tmp_utf8 && (tmp_mskanji || tmp_big5))
      return -1;
    if (!tmp_mskanji && !tmp_big5 && !tmp_utf8)
      for (j = 0;;)
	{
	  if (j >= i)
	    return -1;
	  if (!initstring[j++])
	    break;
	}
    if (mode == 2)
      return 0;
    if (this == NULL)
      return -1;
    initstring[i++] = 0;
    initstring[i++] = 0;
    initstring[i++] = 0;
    initstring[i++] = 0;
    initstring[i++] = 0;
    initstring[i++] = 0;
    initstring[i++] = 0;
    if (mode == 1)
      {
	if ((tmp_mskanji || tmp_big5 || tmp_utf8 ||
	     !memcmp (init_string, initstring, i))
	    && tmp_win95flag == this->win95flag
	    && tmp_lockgr == this->rcv.lockgr
	    && tmp_ssgr == this->rcv.ssgr
	    && tmp_mskanji == (this->rcv.lgr.type == MS_KANJI && this->rcv.gr == &this->rcv.lgr)
	    && tmp_big5 == (this->rcv.lgr.type == BIG5 && this->rcv.gr == &this->rcv.lgr)
	    && tmp_utf8 == (this->rcv.lgr.type == UTF8 && this->rcv.gr == &this->rcv.lgr))
	  return 0;
      }
    memcpy (init_string, initstring, i);
  }
  iso2022_win95flag = this->win95flag = tmp_win95flag;
  init (&this->rcv);
  init (&this->trns);
  if (tmp_mskanji)
    {
      this->rcv.lgr.type = MS_KANJI;
      this->rcv.lgr.len = 2;
      this->rcv.gr = &this->rcv.lgr;
      this->trns.lgr = this->rcv.lgr;
      this->trns.gr = &this->trns.lgr;
    }
  if (tmp_big5)
    {
      this->rcv.lgr.type = BIG5;
      this->rcv.lgr.len = 2;
      this->rcv.gr = &this->rcv.lgr;
      this->trns.lgr = this->rcv.lgr;
      this->trns.gr = &this->trns.lgr;
    }
  if (tmp_utf8)
    {
      this->rcv.lgr.type = UTF8;
      this->rcv.lgr.len = 0;
      this->rcv.gr = &this->rcv.lgr;
      this->trns.lgr = this->rcv.lgr;
      this->trns.gr = &this->trns.lgr;
    }
  for (i = 0 ; this->initstring[i] ; i++)
    put (&this->rcv, this->initstring[i]);
  for (i++ ; this->initstring[i] ; i++)
    put (&this->trns, this->initstring[i]);
  if (tmp_lockgr)
    {
      this->rcv.lgr = *this->rcv.gr;
      this->rcv.gr = &this->rcv.lgr;
      this->rcv.lockgr = 1;
    }
  if (tmp_ssgr)
    this->trns.ssgr = 1;
  return 0;
}

int
iso2022_init_test (char *p)
{
  return iso2022_init (NULL, p, 2);
}

int
xMultiByteToWideChar (UINT a1, DWORD a2, LPCSTR a3, int a4, LPWSTR a5, int a6)
{
  static int (WINAPI *f)(UINT, DWORD, LPCSTR, int, LPWSTR, int) = 0;
  int i, j, k;
  unsigned char c;
  unsigned long w;

  if (!f)
    f = (int (WINAPI*)(UINT, DWORD, LPCSTR, int, LPWSTR, int)) GetProcAddress (GetModuleHandle ("kernel32"), "MultiByteToWideChar");
  if (a1 != CP_UTF8)
    return f (a1, a2, a3, a4, a5, a6);
  if (a4 == -1)
    a4 = lstrlenA (a3);
  k = 0;
  a6--;
  for (i = 0 ; i < a4 ; i++)
    {
      c = a3[i];
      if ((c & 0x80) == 0)
	j = 1, w = c;
      else if ((c & 0xe0) == 0xc0)
	j = 2, w = c & 0x1f;
      else if ((c & 0xf0) == 0xe0)
	j = 3, w = c & 0x0f;
      else if ((c & 0xf8) == 0xf0)
	j = 4, w = c & 0x07;
      else if ((c & 0xfc) == 0xf8)
	j = 5, w = c & 0x03;
      else if ((c & 0xfe) == 0xfc)
	j = 6, w = c & 0x01;
      else
	continue;
      if (i + j <= a4)
	{
	  while (--j)
	    w = (w << 6) | (a3[++i] & 0x3f);
	  if (w >= 0x10000UL)
	    w = (unsigned long)L'?';
	  if (k < a6)
	    a5[k] = (WCHAR) w;
	  else if (k == a6)
	    break;
	  k++;
	}
      else
	break;
    }
  if (k <= a6)
    a5[k] = 0;
  return k + 1;
}

int
xWideCharToMultiByte (UINT a1, DWORD a2, LPCWSTR a3, int a4, LPSTR a5, int a6,
		     LPCSTR a7, LPBOOL a8)
{
  static int (WINAPI *f)(UINT, DWORD, LPCWSTR, int, LPSTR, int, LPCSTR,
			 LPBOOL) = 0;
  int i, j, k;
  WCHAR w;
  unsigned char b[10];

  if (!f)
    f = (int (WINAPI*)(UINT, DWORD, LPCWSTR, int, LPSTR, int, LPCSTR, LPBOOL)) GetProcAddress (GetModuleHandle ("kernel32"), "WideCharToMultiByte");
  if (a1 != CP_UTF8)
    return f (a1, a2, a3, a4, a5, a6, a7, a8);
  if (a4 == -1)
    a4 = lstrlenW (a3);
  k = 0;
  a6--;
  for (i = 0 ; i < a4 ; i++)
    {
      w = a3[i];
      if (w < 0x80)
	j = 1, b[0] = (unsigned char) w;
      else if (w < 0x800)
	j = 2, b[1] = 0x80 | (w & 0x3f), b[0] = 0xc0 | ((w >> 6) & 0x1f);
      else
	j = 3, b[2] = 0x80 | (w & 0x3f),
	  b[1] = 0x80 | ((w >> 6) & 0x3f), b[0] = 0xe0 | ((w >> 12) & 0x0f);
      if (k + j <= a6 || a6 == -1)
	{
	  if (a6 == -1)
	    k += j;
	  else
	    {
	      a5[k++] = b[0];
	      if (j >= 2)
		a5[k++] = b[1];
	      if (j == 3)
		a5[k++] = b[2];
	    }
	}
      else
	break;
    }
  if (k <= a6)
    a5[k] = '\0';
  return k + 1;
}
