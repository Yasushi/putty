...
void
l10nTextOut (HDC hdc, int x, int y, char *str, int cnt)
{
  char *a = (char *)str;
  char b[256];

  if (getEnabled() && getl10nstr (a, "", b, 256))
    a = b;
  TextOutA (hdc, x, y, a, strlen (a));
}

BOOL
l10nGetTextExtentPoint32 (HDC hdc, char *str, int len, LPSIZE s)
{
  char *a = (char *)str;
  char b[256];
  ...
  if (getEnabled() && getl10nstr (a, "", b, 256))
    a = b;
  return GetTextExtentPoint32 (hdc, a, strlen (a), s);
}
