...
DWORD WINAPI do_main(LPVOID lParam)
{
	...
	int nodelay = param->cfg.tcp_nodelay &&
	    (GetFileType(GetStdHandle(STD_INPUT_HANDLE)) == FILE_TYPE_CHAR);


	error = ssh_backend.init(NULL, &tld.backhandle, &param->cfg, param->cfg.host, param->cfg.port,
	 &realhost, nodelay, param->cfg.tcp_keepalives);
	if (error) {
	...
	logctx = log_init(NULL, &param->cfg);
...
int get_userpass_input(prompts_t *p, unsigned char *in, int inlen) {
    ...
    strncpy(p->prompts[0]->result, cmdline_get_line_password, p->prompts[0]->result_len);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	/* plink パラメータセット */
	do_defaults(m_szHost, &m_tParam.cfg);
	if (m_tParam.cfg.host[0] == '\0') {
	    /* No settings for this host; use defaults */
	    strncpy(m_tParam.cfg.host, m_szHost, sizeof(m_tParam.cfg.host) - 1);
	    m_tParam.cfg.host[sizeof(m_tParam.cfg.host) - 1] = '\0';
	    m_tParam.cfg.port = 22;
	}
	m_tParam.cfg.x11_forward = 0;
	m_tParam.cfg.agentfwd = 0;
	m_tParam.cfg.nopty = 1;
	m_tParam.cfg.ssh_no_shell = 1;
	if (port > 0)
	    m_tParam.cfg.port = port;
	if (protocol == 1) {
	    m_tParam.cfg.sshprot = 0;	       /* ssh protocol 1 only */
	}else if (protocol == 2) {
	    m_tParam.cfg.sshprot = 3;	       /* ssh protocol 2 only */
	}
	if (m_szUser[0] != '\n') {
	    strncpy(m_tParam.cfg.username, m_szUser, sizeof(m_tParam.cfg.username));
	    m_tParam.cfg.username[sizeof(m_tParam.cfg.username) - 1] = '\0';
	}
	/* プライベートキーファイル名を取得 (フルパス) */
	if (m_szKeyFile[0] == '\n'){
	    m_tParam.cfg.keyfile.path[0] = '\0';
	}else if(m_szKeyFile[1] == ':'){
	    strcpy(m_tParam.cfg.keyfile.path, m_szKeyFile);
	}else if(m_szKeyFile[0] == '\\'){
	    strcpy(m_tParam.cfg.keyfile.path, m_szKeyFile);
	}else if((m_szKeyFile[0] == '.') && (m_szKeyFile[1] == '\\')){
	    sprintf(m_tParam.cfg.keyfile.path, "%s\\%s", m_szCurDir, &m_szKeyFile[2]);
	}else{
	    sprintf(m_tParam.cfg.keyfile.path, "%s\\%s", m_szCurDir, m_szKeyFile);
	}
	...
	if(m_szPasswd[0] == '@'){
	    ...
	    Password(m_tParam.cfg.keyfile.path, PWD_ENCODE, &m_szPasswd[1], strlen(m_szPasswd) - 1, pwd, sizeof(pwd));
	    WritePrivateProfileString("SSH", "Password", pwd, m_szIniFile);
	    set_cmdline_get_line_password(&m_szPasswd[1]);
	}else if (m_szPasswd[0] != '\0') {
	    char pwd[64];
	    /* 暗号化パスワード ⇒ 平文パスワード */
	    Password(m_tParam.cfg.keyfile.path, PWD_DECODE, m_szPasswd, strlen(m_szPasswd), pwd, sizeof(pwd));
	    set_cmdline_get_line_password(pwd);
	}

	if (compress)
	    m_tParam.cfg.compression = 1;
	i = 0;
	p = m_tParam.cfg.portfwd;
	end = m_tParam.cfg.portfwd + sizeof m_tParam.cfg.portfwd - 1;
	while (1) {
	    char type;
	    char* param;
	    int len;
		...
		*p++ = type;
		...
		MessageBox(NULL, "out of space for port forwardings!", APPNAME, MB_OK | MB_ICONERROR);
		return -1;
	    }
	}
	*p = '\0';
	if (m_tParam.cfg.portfwd[0] == '\0') {
