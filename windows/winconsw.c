...
static HWND create_control(HWND dialog, int ctrlID, long style, long exstyle, const char* windowClass, const char* label, RECT* r) {
    HWND window;
    HINSTANCE instance = (HINSTANCE) GetWindowLong(dialog, GWL_HINSTANCE);
...
HWND create_edit(HWND dialog, int ctrlID, prompt_t* prompt, RECT r) {
    ...
    SendMessage(window, EM_LIMITTEXT, prompt->result_len - 1, 0);
    return window;
}
...
BOOL CALLBACK getline_dialog_proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
        {
            HINSTANCE instance = (HINSTANCE) GetWindowLong(hwndDlg, GWL_HINSTANCE);
    ...
    case WM_COMMAND:
        ...
        case MAKEWPARAM(IDOK, BN_CLICKED):
            {
                int i;
                prompts_t *p = (prompts_t *) GetWindowLong(hwndDlg, GWL_USERDATA);
                for (i = 0; i < (int)p->n_prompts; i++) {
                    GetDlgItemText(hwndDlg, IDC_RESULT_0 + i, p->prompts[i]->result, p->prompts[i]->result_len);
                }
